...
export function generatePDF(filledPreviewContent, signatureDataUrl) {
    ...
    // Convert the signature image (base64 PNG) before inserting it into the content
    const contentWithSignature = filledPreviewContent.replace(
      '{{CUSTOMER_SIGNATURE_PATH}}',
      `<img src="${signatureDataUrl}" alt="Customer Signature" width="300px" height="150px" crossorigin="anonymous" />`
    );
    ...
    // Ensure the signature is properly rendered before converting it to a canvas
    setTimeout(() => {
      html2canvas(tempDiv, {
